...
if ! ssh-add -L >/dev/null 2>&1; then
  ...
  ssh-agent > ~/.ssh/agent.env >/dev/null 2>&1
  source ~/.ssh/agent.env >/dev/null 2>&1
...
# enable starship bash prompt, if the package is installed
# an alias makes install easier
alias starship_install="curl -fsSL https://starship.rs/install.sh | sh"
